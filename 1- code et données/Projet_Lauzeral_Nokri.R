...
acci<-read.table(file="C:/Users/amale/Documents/GitHub/Statistique--Bayesienne/1- code et données/plane_accidents.txt", head=TRUE)
require(stats4)
library(lubridate)
library(plyr)
...
# Bayesian estimator from discrete prior
...
# We plot the posterior distribution of theta|X1..Xn corresponding to the gamma a priori
#Post uniform
...
# An a priori Gamma density of theta that we have chosen
...
plot(xfitPostGw,yfitPostGw,type='l', xlim=c(0,1.3), ylim=c(0,10))
...
# -> the mean set to the MLE estimates
...
for(i in 1:500)
  ...
  mean(bootstw)
#500 samples with size=212 and mean=0.6686226 (MLE for week)
...
mean(tw) #MLE mean
# Theta is predicted to be 0.6686226 by the maximum likelihood estimator

biasMLEw= mean(tw)-mleW
biasMLEw
# Low but negative bias (-0.001188679) -> MLE underestimates theta
...
rmseMLEw
# RMSE is low but positive as well (0.05618556)
...
mean(B1w)
# Bayes with discrete prior predicts theta to be 0.6733396

biasB1=mean(B1w)-summary(postw)$mean
biasB1
# low but negative bias (-0.001188685) as well as MLE

rmseB1w=rmse(B1w,summary(postw)$mean)
rmseB1w
# approximately the same RMSE as MLE (0.05618556)
...
mean(B2w)
# theta is estimated to be 0.669217 here
...
for(i in 1:500)
  ...
  mean(bootstm)
# 500 samples with size = 48 and mean=2.9605 (Monthly average of plane accidents over a month estimated by ML method)

# Maximum likelihood
tm=apply(bootstm,1,mean) #A vector of MLE from each sample

mean(tm) #MLE mean
# Theta is predicted to be 2.9605 by the maximum likelihood estimator

biasMLEm= mean(tm)-mleM
biasMLEm
# Low but positive bias (0.002166667 vs -0.004125) -> MLE underestimates theta
...
mean(B1m)
# Bayes with discrete prior predicts theta to be 2.981333

biasB1m=mean(B1m)-summary(postm)$mean
biasB1m
# really low bias (close to 0)
...
mean(B2m)
# theta is estimated to be 2.959417 here
